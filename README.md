...
- **Strings and interpolation** &mdash; expandable strings `"$var $(expr)"`, escaped quotes `""`, `#` in interpolated strings, verbatim strings, here-strings `@"` / `@'`, sub-expressions `$(...)`, array sub-expressions `@(...)`, hashtable literals `@{}`, and PowerShell-style escaping
